...
func TestMiddleware_Handle_NoContextKey(t *testing.T) {
	...
	if resp.StatusCode != fiber.StatusInternalServerError {
		t.Errorf("Expected status code %d for missing context key, but got %d", fiber.StatusInternalServerError, resp.StatusCode)
	}
	...
	expectedErrorMessage := "ContextKey is not set"
